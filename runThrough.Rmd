...
list.of.packages <- c(
    "stringr","ggplot2",  "here",  "knitr", "tidyr", "reshape2", "readr", "dplyr", "devtools", "kableExtra", "stargazer", "nlme", "mgcv", "gratia")
...
```{r groupSpecies, include = F, echo=F, warning=F, message=F}
...
```{r sppGroupTab, include = T, echo=F, warning=F, message=F}
...
ng two GAMs. First, we aggregated species into groups, and (see \@ref(tab:sppGroupTab)). Next, we analyzed individual species trends. 
We analyzed individual species’ trends using the a generalized additive model: 

\begin{equation} 
g(E(y_i)) = \beta_0 f(year_i) + \epsilon_i, \\
y_i ~ some exp. distribution
\end{equation} 

- Most species trends were non-linear over the study period. GAM optimizes the residuals by fitting knots in the model (count, response value), so if the trend is linear, it will fit it.
- ARound 1980 the hours effort and number of participants increased linearally, therefore we did not fit a smooth to the hours effort (see \@ref(fig:effortPlots)). 
...
- "for a Poisson model the expected value after log transformation is modelled to be linearly dependent on the explanatory variables""
- 
...
gam.results.ind <- runGams(df = birds.grouped, family = "quasipoisson",
                           group.var = "ind",
                           rds.filename =  "indSum.withFixedEf",
                           mod.formula  = as.formula("value ~ s(year, by = \"key\") + key + hours"),
                           fig.out.path = NULL,
                           year.start = 1973,
                           zero.thresh = 0.11,
                           length.out = 200)
plot(gam.results.ind[[1]])
# Run the gam for species (ind by spp)
...
gam.results.key     <-readRDS(paste0(here::here(),"/Results/gamResults_key2019-02-23.rds"))
gam.results.ind     <-readRDS(paste0(here::here(),"/Results/gamResults_ind2019-02-23.rds"))
gam.results.ind.sum <-readRDS(paste0(here::here(),"/Results/gamResults_indSum2019-02-23.rds"))
...
We controlled for stationarity by taking second differences (i.e., using an AR model with lag = 2).

```{r sppAnalysedTab, include = T, echo=F, warning=F, message=F}

# Maake the table
 unique(names(gam.results.key)) %>% 
  kable(format    = "latex", 
      longtable = T, 
      booktabs  = T, 
      caption   = "Species with sufficient data for analysis") %>% 
    kable_styling(full_width = F, font_size = 9)#, latex_options = c("repeat_header"))
```
