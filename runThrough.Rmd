...
list.of.packages <- c(
    "stringr","ggplot2",  "here",  "knitr", "tidyr", "reshape2", "readr", "dplyr", "devtools", "kableExtra", "stargazer")
...
if(is.factor(birds.grouped$ind)){
    index <- levels(birds.grouped$ind)}else(index <- unique(birds.grouped))
groupGams <- list()
for (i in 1:index) {
    gam.data <- birds.grouped %>% 
        # keep species grouping i
        # filter(ind == index[i], 
        #        year > startYear)  # %>%
        group_by(year) %>%
        mutate(value = sum(value)) %>%
        distinct(year, hours,value )
    #     
    groupGams[[i]] =
        gamm(
            value ~ s(year) + hours,
            ...
            # Beacuse time series are usually autocorrelated, the standard errors may be deflated. Let's look at ACF
            ## look at autocorrelation in residuals:
        acf(resid(groupGams[[i]]$lme, type = "normalized"))
        ## ...wait... look at the plot, only then do...
        pacf(resid(groupGams.AR1[[i]] $lme, type = "normalized"))
...
## ...wait... look at the plot, only then do...
pacf(resid(ar1.gam$lme, type = "normalized"))
pacf(resid(ar2.gam$lme, type = "normalized"))
pacf(resid(ar3.gam$lme, type = "normalized"))

aov.tab = anova(ar1.gam$lme, ar2.gam$lme, ar3.gam$lme)
      
# Keep the best fitting model
mod.keep = aov.tab %>% 
    slice(which.min(`p-value`)) %>% 
    mutate(modelName = paste0("ar", Model, ".gam")) %>% 
    select(modelName, df, AIC, BIC, logLik, L.Ratio, "p-value")



    }

names(groupGams) <- index
