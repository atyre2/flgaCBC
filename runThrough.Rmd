...
devtools::install_github("trashbirdecology/flgaCBC", force = T)
library(flgaCBC)
```
...
birds.grouped <- full_join(birds, birds.new %>% rename(species = values) %>%  na.omit(family))
birds.grouped
...
for (i in 1:index) {
    ...
    groupGams[[i]] =
        gam(
            # log10(count+1.0e-50/Hours) ~  s(Year) ,
            count ~ s(Year) + s(Hours),
            method = "REML",
            family = poisson,
            data = myDF[[i]] %>% filter(Year >= 1985)
            # scale = 1,
            # gamma = gamma
        )
}

names(groupGams) <- names(myDF)
